...
public class Wyszukanie_tax extends taxowki_polozenie{  //klasa do wszukania i wyswietlenia 5 wyselekcjonowanych taxowek 
	private final int SIZE_NEW = 5;
	private double tab_odl_tax_ws[] = new double[SIZE_NEW];  //tablice wyselekcjonowanych tablic
	...
	private void wybor_tax(){  //metoda wybierajaca 5 dostepnych taxowek w odleglosci 1000 m
		...
		for(int i = 0; i < tab_odl_tax.length; i++){
			if(tab_odl_tax[i]< 1000 && zaj_tax[i] == false){
				lista_totw.add(tab_odl_tax[i]);              // przepisywanie wartosci spelniajacych wymagania z tablic do list
				lista_ntw.add(nazwa_tax[i]);
			}
		}
		
		quicksort(lista_ntw,lista_totw,0,lista_totw.size()-1);  //wywolanie sortowania
		
		for(int i = 0; i < SIZE_NEW; i++){                         // wybranie 5 elementow od najmniejszej odleglosci do najwiekszej
			tab_odl_tax_ws[i] = lista_totw.get(i).doubleValue();  //wywolanie metody zamieniajacej klase opakowujaca Double na typ podstawowy double
			nazwa_tax_ws[i] = lista_ntw.get(i);
		}
	}
	
	private void quicksort(List<String> lista_ntw, List<Double> lista_totw, int left, int right){ // metoda qs sortujaca elementy
		int i,j;
		Double x,y;
		String z;
		i = left;
	...
	public void wyswietl_tax(){  //? ma wywolywac obliczenia() a potem wybor_tax() i wyswietlac tablice 5 elementow
		PrintWriter konsola = new PrintWriter(System.out, true);   //inicjalizacja strumienia znakowego o nazwie konsola
		Date data = new Date();
		obliczenia();   
		wybor_tax();
